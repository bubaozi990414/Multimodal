...
class Visualizer():
    ...
    def display_current_results(self, visuals, total_iters, epoch, save_result):
        """Display current result on tensorboad; save current result to an HTML file.

        Parameters:
            visuals (OrderedDict) - - dictionary of images to display or save
            total_iters (int) -- total iterations
            epoch (int) - - the current epoch
            save_result (bool) - - if save the current result to an HTML file
        """
...
class MyVisualizer:
    def __init__(self, opt):
        ...
        self.name = opt.name
        self.img_dir = os.path.join(opt.checkpoints_dir, opt.name, 'result')
