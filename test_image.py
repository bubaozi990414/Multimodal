...
parser.add_argument('--images-dir', type=str, default='./input/main_image')#,required=True)
parser.add_argument('--result-dir', type=str, default='./result/mask')#required=True)
parser.add_argument('--gt-dir', type=str, default=None)
parser.add_argument('--pretrained-weight', type=str, default="./checkpoints/SGHM-ResNet50.pth")#,required=True)
...
for i in range(num_image):
    ...
    image_name = image_path[image_path.rfind('e')+1:image_path.rfind('.')]
    ...
    # inference
    pred_alpha, pred_mask = inference2.single_inference(model, img)
    ...
    save_path = args.result_dir + '/' + image_name + '_new.png'
    Image.fromarray(((pred_alpha * 255).astype('uint8')), mode='L').save(save_path)
